    ...
    <div class="py-12">
        <div class="max-w-lg mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('menu.update', $producto) }}" enctype="multipart/form-data">
                    @csrf @method('PUT')
                    ...
                    <div class="flex justify-center mb-6">
                        <div class="relative group">
                            <div
                                ...
                                @if ($producto->imagen)
                                    <img id="preview-img" src="{{ asset('storage/' . $producto->imagen) }}"
                                        class="w-full h-full object-cover">
                                @else
                                    <img id="preview-img" class="w-full h-full object-cover hidden">
                                    <span id="default-text" class="text-gray-400 text-sm text-center px-2">Sin
                                        imagen</span>
                                @endif
                            </div>
                            <label for="imagen"
                    ...
                    <div class="flex items-center justify-center mt-6">
                        <button type="button" onclick="window.location='{{ route('menu.index') }}'"
                            class="mr-4 px-4 py-2 bg-red-800 dark:bg-red-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-red-800 uppercase tracking-widest hover:bg-red-700 dark:hover:bg-white transition ease-in-out duration-150">
                            Cancelar
                        </button>
