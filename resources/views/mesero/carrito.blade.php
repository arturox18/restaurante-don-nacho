        ...
        <div class="max-w-md mx-auto px-4">
            ...
            <div class="flex items-center justify-between mb-6">
                <a href="{{ route('mesero.dashboard') }}"
                    class="p-2 bg-white dark:bg-gray-800 rounded-full shadow-sm hover:bg-gray-100 dark:hover:bg-gray-700 transition flex-shrink-0">
                    <svg class="w-6 h-6 text-gray-600 dark:text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </a>
                
                <h2 class="text-2xl font-bold text-center text-gray-800 dark:text-gray-200 flex-grow">
                    Mesa: {{ $mesa->nombre }}
                </h2>
                    ...
                    @else
                        <div class="flex flex-col gap-3 mt-4">
                            ...
                            <form action="{{ route('mesero.confirmar', $mesa) }}" method="POST" class="grid grid-cols-2 gap-2 mb-2">
                                @csrf
                                <button type="submit" name="accion" value="cocina"
                                    class="bg-gray-800 hover:bg-gray-900 text-white text-sm font-bold py-3 rounded-xl shadow transition transform active:scale-95 flex items-center justify-center gap-1">
                                    Extra a cocina
                                </button>
                                <button type="submit" name="accion" value="cuenta"
                                    class="bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 text-sm font-bold py-3 rounded-xl shadow transition transform active:scale-95 flex items-center justify-center gap-1">
                                    Extra a cuenta
                                </button>
                            </form>
